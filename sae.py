...
class BaseAutoencoder(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.b_dec = nn.Parameter(torch.zeros(self.args.act_size))
        self.b_enc = nn.Parameter(torch.zeros(self.args.dict_size))
        self.W_enc = nn.Parameter(torch.nn.init.kaiming_uniform_(torch.empty(self.args.act_size, self.args.dict_size)))
        self.W_dec = nn.Parameter(torch.nn.init.kaiming_uniform_(torch.empty(self.args.dict_size, self.args.act_size)))
        self.W_dec.data[:] = self.W_enc.t().data
        self.W_dec.data[:] = self.W_dec / self.W_dec.norm(dim=-1, keepdim=True)
        self.num_batches_not_active = torch.zeros((self.args.dict_size,)).to(self.args.device)

        self.to(self.args.dtype).to(self.args.device)

    def preprocess_input(self, x):
        if self.args.get("input_unit_norm", False):
...
class BatchTopKSAE(BaseAutoencoder):
    ...
    def forward(self, x):
        ...
        acts_topk = torch.topk(acts.flatten(), self.args.top_k * x.shape[0], dim=-1)
    ...
    def get_loss_dict(self, x, x_reconstruct, acts, acts_topk, x_mean, x_std):
        ...
        l1_loss = self.args.l1_coeff * l1_norm
        l0_norm = (acts_topk > 0).float().sum(-1).mean()
        aux_loss = self.get_auxiliary_loss(x, x_reconstruct, acts)
        loss = l2_loss + l1_loss + aux_loss
        num_dead_features = (self.num_batches_not_active > self.args.n_batches_to_dead).sum()
    ...
    def get_auxiliary_loss(self, x, x_reconstruct, acts):
        dead_features = self.num_batches_not_active >= self.args.n_batches_to_dead
        if dead_features.sum() > 0:
            residual = x.float() - x_reconstruct.float()
            acts_topk_aux = torch.topk(
                acts[:, dead_features],
                min(self.args.top_k_aux, dead_features.sum()),
                dim=-1,
            )
            acts_aux = torch.zeros_like(acts[:, dead_features]).scatter(-1, acts_topk_aux.indices, acts_topk_aux.values)
            x_reconstruct_aux = acts_aux @ self.W_dec[dead_features]
            l2_loss_aux = self.args.aux_penalty * (x_reconstruct_aux.float() - residual.float()).pow(2).mean()
            return l2_loss_aux
        else:
...
class TopKSAE(BaseAutoencoder):
    ...
    def forward(self, x):
        ...
        x_cent = x - self.b_dec
        acts = F.relu(x_cent @ self.W_enc)
        acts_topk = torch.topk(acts, self.args.top_k, dim=-1)
...
class JumpReLUSAE(BaseAutoencoder):
    def __init__(self, args):
        super().__init__(args)
        self.jumprelu = JumpReLU(feature_size=args.dict_size, bandwidth=args.bandwidth, device=args.device)
    ...
    def get_loss_dict(self, x, x_reconstruct, acts, x_mean, x_std):
        ...
        l0 = StepFunction.apply(acts, self.jumprelu.log_threshold, self.args.bandwidth).sum(dim=-1).mean()
        l0_loss = self.args.l1_coeff * l0
        l1_loss = l0_loss
